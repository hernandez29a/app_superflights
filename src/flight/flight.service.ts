...
import { ILocation } from 'src/common/interfaces/locatio.interface';
...
export class FlightService {
    ...
    async getLocation(desnitationCity: string): Promise<ILocation>{
        const {data} = await axios.get(`https://www.metaweather.com/api/location/search/?query=${desnitationCity}`);
        return data[0];
    }
    ...
    async update(id: string, flightDOT: FlightDTO): Promise<IFlight>{
        return await this.model.findByIdAndUpdate(id, flightDOT,{new: true});
    }

    async delete(id: string){
        await this.model.findByIdAndDelete(id);
        return {status: HttpStatus.OK, msg: 'Vuelo Eliminado '};
    }

    async addPassenger(flightId: string, passengerId: string): Promise<IFlight>{
        return await this.model.findByIdAndUpdate(flightId, {
            $addToSet:{ passengers:passengerId}}, {new: true})
            .populate('passengers');
    }
